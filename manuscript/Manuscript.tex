...
data sets can be found in Figure 2. Both figures show that the marginal
distributions of the bootstrapped data and the synthetic data are highly
similar. Additionally, the univariate descriptives for the original data
and the synthetic data can be found in Table 2.
...
\caption{Empirical sampling distribution of the number within each
category of the categorical variables in the bootstrapped and synthetic
boys data.}
\end{figure}
...
incorporating the additional uncertainty that is due to synthesizing
(part of) the data, potential disclosure risks that remain after
synthesizing the data, and the utility of the synthetic data. First, the
procedure of generating multiple synthetic sets may seem overly
complicated. We would like to emphasize that analyzing a single
...
package, like \texttt{mice}. After this, \texttt{synthpop} can be used
...
synthetic sets would be sufficient. However, as of today, no pooling
rules for one-step imputation of missingness and synthetisation have
been developed. The derivation of those would yield a massive
theoretical improvement to further reduce the burden of creating
synthetic data sets.
...
of synthetic data \citep[e.g.,][]{snoke2018general}, while \texttt{mice}
contains methodology to allow for flexible synthesis of intricate data
structures, such as multilevel data. Being able to use such
functionality interchangeably would greatly benefit data analysist.
