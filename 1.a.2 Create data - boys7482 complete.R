...
meth <- make.method(AGD::boys7482)
pred <- make.predictorMatrix(boys)
meth["bmi"] <- "~ I(wgt / (hgt/100)^2)"

pred[c("wgt", "hgt"), "bmi"] <- 0

imp7482 <- mice(AGD::boys7482, meth = meth, pred = pred,      ## Impute the boys dataset by means of a single
            m = 1, printFlag = F)                    ## imputation, and fill in the imputed data into
boyscomp7482 <- complete(imp7482)                        ## the dataset containing missing values.
...
part <- resample_partition(boyscomp7482, partitions) ## partition the data in five equal parts