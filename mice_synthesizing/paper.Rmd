...
Since multiple imputation of synthetic data is closely linked to multiple imputation for missing data, using existing missing data software that research are familiar with already, would dramatically reduce the effort of creating synthetic data. The R-package `mice` [@mice] implements multiple imputation for missing data in a straightforward and user-friendly manner, but is not restricted to the imputation of missing data. Since `mice` can be easily adjusted to impute all observed values, or a subset hereof, we propose that it is able to ease the creation of synthetic datasets. 
